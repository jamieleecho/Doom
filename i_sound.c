...
I_SubmitSound(void)
{
  ...
  static float buff[SAMPLECOUNT * 8];
  int ii, jj;
  
  for (ii=jj=0; ii<SAMPLECOUNT * 2;) {
    float left = mixbuffer[ii++]/32768.0f;
    float right = mixbuffer[ii++]/32768.0f;
    buff[jj++] = left;
    buff[jj++] = right;
    buff[jj++] = left;
    buff[jj++] = right;
  ...
  jlcQueueWrite(audio_queue, buff, sizeof(buff));
}
...
OSStatus soundOutProc (AudioDeviceID inDevice, const AudioTimeStamp* inNow, const AudioBufferList* inInputData,
    ...
    int cnt = jlcQueueRead(audio_queue, buf, size);
    memset(buf + cnt, 0, size - cnt);
    // for (cnt=0; cnt<size/4; cnt++)
    //     buf[cnt] = (float)cnt/(float)size;
...
{ 
  ...
  audio_queue = jlcQueueAlloc(0x10000);
  soundfx_init();
